...
export default class Launcher extends EventEmitter {
    ...
    if (!gotSingleLock) {
      ...
      app.on('second-instance', (event, argv, workingDirectory) => {
        ...
        if (!is.macOS() && argv.length > 1) {
          this.handleAppLaunchArgv(argv) // deal with launch app by commandLine
        }
      })
  ...
  init () {
    ...
    logger.info('[Dockeron] openedAtLogin:', this.openedAtLogin)
  ...
  handleOpenUrl () {
    ...
    app.on('open-url', (event, url) => {
      logger.info(`[Dockeron] open-url: ${url}`)
      event.preventDefault() // stop default event handle
      this.url = url
  ...
  handleOpenFile () {
    ...
    app.on('open-file', (event, path) => {
      logger.info(`[Dockeron] open-file: ${path}`)
  ...
  handleAppWillQuit () {
    app.on('will-quit', () => {
      logger.info('[Dockeron] will-quit')
      if (global.application) {
