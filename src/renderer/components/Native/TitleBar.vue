<template>
  <div class="title-bar">

    <div class="title-bar-dragger">
          <Icon type="ios-boat"></Icon> dockeron
    </div>
    <ul  class="window-actions">
      <li @click="handleMinimize">
        <fa-icon name="win-minimize" width="12" height="12" />
      </li>
      <li @click="handleMaximize">
        <fa-icon name="win-maximize" width="12" height="12" />
      </li>
      <li @click="handleClose" class="win-close-btn">
        <fa-icon name="win-close" width="12" height="12" />
      </li>
    </ul>
  </div>
</template>

<script>
  import Icon from '@/components/Icons/SvgIcon'
  import '@/components/Icons/win-minimize'
  import '@/components/Icons/win-maximize'
  import '@/components/Icons/win-close'


  export default {
    name: 'mo-title-bar',
    components:{
        [Icon.name]: Icon
    },
    props: {
    },
    computed: {
      win: function () {
        return this.$electron.remote.getCurrentWindow()
      }
    },
    methods: {
      handleMinimize: function () {
        this.win.minimize()
      },
      handleMaximize: function () {
        if (this.win.isMaximized()) {
          this.win.unmaximize()
        } else {
          this.win.maximize()
        }
      },
      handleClose: function () {
        //this.win.close()
        this.$electron.ipcRenderer.send('event', 'application:quit')
      }
    }
  }
</script>

<style lang="scss">
.title-bar {
  position: fixed;
  top: 0;
  left: 0;
  display: flex;
  flex-direction: row;
  width: 100%;
  height: 36px;
  z-index: 5000;
  .title-bar-dragger {
    flex: 1;
    user-select: none;
    -webkit-app-region: drag;
    -webkit-user-select: none;
  }
  .window-actions {
    opacity: 0.4;
    transition: $--fade-transition;
    list-style: none;
    padding: 0;
    margin: 0;
    z-index: 5100;
    > li {
      display: inline-block;
      padding: 5px 15px;
      margin: 0;
      color: $--titlebar-actions-color;
      &:hover {
        background-color: $--titlebar-actions-active-background;
      }
      &.win-close-btn:hover {
        color: $--titlebar-close-active-color;
        background-color: $--titlebar-close-active-background;
      }
    }
  }
  &:hover {
    .window-actions {
      opacity: 1;
    }
  }
}
</style>
